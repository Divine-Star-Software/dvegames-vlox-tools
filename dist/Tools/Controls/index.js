...
export default function (graph) {
    ...
    ToolPanelViews.registerView("Controls", (component) => {
        const schemaInstance = Schema.CreateInstance(BooleanProp("enablePointerLock", {
            name: "Enable Pointer Lock",
            value: false,
            initialize: (node) => node.observers.set.subscribe(() => setPointerLock(node.get())),
        }), BooleanProp("enableCrossHairs", {
            name: "Enable Cross Hairs",
            value: false,
            initialize: (node) => node.observers.set.subscribe(() => setCrossHairs(node.get())),
        }));
        return frag(SchemaEditor({
            schemaInstance,
        }));
    });
}
