...
SchemaEditorInputRegister.register(BooleanPropertyInput, BooleanPropertyInput.createPropertyRenderFC((props) => {
    const { node } = props;
    ...
    return SEInputBase(props, elm("input", {
        ...
        onchange: ({ target }) => {
            node.update(Boolean(target.checked));
        },
        signal: updateInput((elm) => (elm.checked = Boolean(node.get()))),
    }));
}));
