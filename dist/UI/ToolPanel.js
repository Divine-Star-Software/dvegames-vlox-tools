...
`);
export default function ToolPanel(component) {
    ...
    }, elm("div", "view-select-container", elm("select", {
        value: "",
        onchange(event) {
            const selectedViewId = event.target.value;
            showView(selectedViewId);
        },
    }, elm("option", { value: "" }, ""), ToolPanelViews.getViews().map((viewId) => elm("option", { value: viewId }, viewId)))), viewContainer);
}
